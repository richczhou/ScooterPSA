...
export default function App () {
  return (
  <Canvas gl={{alpha: false}} dpr={[1, 2]} camera={{ position: [0, 0, 10], fov: 35, near: 1, far: 40 }}>
    <color attach="background" args={["#f5fbff"]} />
    <ambientLight intensity={0.5} />
    <spotLight intensity={2} angle={0.2} penumbra={1} position={[30, 50, 30]} castShadow shadow-mapSize={[512, 512]} />
...
function Effects() {
  return (
    // Effect composer needed for the right gamma on background, unsure why?
    <EffectComposer>
    </EffectComposer>
  )
}